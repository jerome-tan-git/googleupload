...
public class DriveCommandLine {
	...
	public DriveCommandLine(String _phantomJsPath, String _fileTitle, String _googleAccount, String _googlePassword) {
		this.phantomJsPath = _phantomJsPath;
		this.fileTitle = _fileTitle;
		this.name = _googleAccount;
		this.pwd = _googlePassword;
	}

	public static void main(String[] args) throws IOException,
			InterruptedException {
		DriveCommandLine driveCommandLine = new DriveCommandLine(
				"C:\\phantomjs\\phantomjs.exe", "Master.twbx","","");
	...
	public void uploadFile(String _fileName) throws IOException,
		...
		try {
			...
			FileUtils.copyFile(screenShotFile, new java.io.File("./a1.png"));
			...
			// FileUtils.copyFile(screenShotFile1, new
			// java.io.File("./a2.png"));
			// System.out.println(2);
			// driver.findElement(By.id(final_id)).click();
			java.io.File screenShotFile2 = ((TakesScreenshot) driver)
					.getScreenshotAs(OutputType.FILE);
			FileUtils.copyFile(screenShotFile2, new java.io.File("./a3.png"));
			// System.out.println(3);
			// screenShotFile =
			// ((TakesScreenshot)driver).getScreenshotAs(OutputType.FILE);
			// FileUtils.copyFile(screenShotFile, new java.io.File("./a.png"));
			// new WebDriverWait(driver, 10).until(new
			// ExpectedCondition<Boolean>() {
			// @Override
			// public Boolean apply(WebDriver d) {
			// System.out.println("invoker this method...");
			// return
			// d.findElement(By.xpath("//body")).getText().contains("put your email name here");
			// }
			// });

			currentUrl = driver.getCurrentUrl();
			// System.out.println(driver.getPageSource());
			System.out.println("request=" + currentUrl);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			driver.quit();
		...
		if (currentUrl.indexOf("code=") != -1) {
			code = currentUrl.substring(currentUrl.indexOf("code=") + 5);
		}
		if (ComUtil.isEmpty(code)) {
			logger.error("Can't get authorization。");
			return;
		}
		...
		if (ifUpdate(service)) {
			...
			String parentId = "0B-egulo89R22OVRESE1oZTVhX3c";
			if (parentId != null && parentId.length() > 0) {
				body.setParents(Arrays.asList(new ParentReference()
						.setId(parentId)));
			}

			java.io.File fileContent = new java.io.File(_fileName);
			FileContent mediaContent = new FileContent("application/x-twb",
					fileContent);

			List<String> deleteFiles = this.getMasterFile(service);
	...
	private boolean ifUpdate(Drive service) {
		boolean result = true;
		...
		try {
			...
			if (fileList != null && fileList.size() > 0) {
				for (File file : fileList) {
					...
					if (file.getTitle().trim().toLowerCase()
							.equals("master.twbx")) {
	...
	private void getFiles(Drive service) {
		try {
			List<File> fileList = retrieveAllFiles(service);
			if (fileList != null && fileList.size() > 0) {
				for (File file : fileList) {
					System.out.println("file title=" + file.getTitle()
							+ ", file id=" + file.getId() + ", file time="
